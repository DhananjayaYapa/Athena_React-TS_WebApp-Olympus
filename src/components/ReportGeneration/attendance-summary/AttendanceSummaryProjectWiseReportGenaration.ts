...
import {
  ATTENDANCE_SUMMARY_CONSTANTS_PROJECT_WISE,
  COMMON_COLORS,
  COMMON_USES_SUMMERY_PROJECT_WISE,
  EXCEL_ATTENDANCE_SUMMARYLEAVE_STATUS,
  EXCEL_ATTENDANCE_SUMMARY_DATE_FLAG,
  EXCEL_COLOR_CODE_FOR_ATTENDANCE_SUMMARY,
  EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE,
  EXCEL_HEADER_COLORS,
  EXCEL_HEADINGS,
  EXCEL_LEAVES_COLORS,
...
const EMP = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.EMP
const CLIENT = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.CLIENT
const PROJECT = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.PROJECT
const EMPLOYEE_NAME = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.EMPLOYEE_NAME
const PROJECT_ROLE = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.PROJECT_ROLE
const GRAND_TOTAL = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.GRAND_TOTAL
const FULL_DAYS = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.FULL_DAYS
const HALF_DAYS = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.HALF_DAYS
const TOTAL_WORKED_DAYS = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.TOTAL_WORKED_DAYS
const TOTAL_LEAVE_DAYS = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.TOTAL_LEAVE_DAYS
const TOTAL_WORKING_DAYS = EXCEL_COLUMN_NAMES_FOR_ATTENDANCE_SUMMARY_PROJECT_WISE.TOTAL_WORKING_DAYS

const X_SPLIT = ATTENDANCE_SUMMARY_CONSTANTS_PROJECT_WISE.X_SPLIT
const Y_SPLIT = ATTENDANCE_SUMMARY_CONSTANTS_PROJECT_WISE.Y_SPLIT
const START_COLUMN_NUMBER = ATTENDANCE_SUMMARY_CONSTANTS_PROJECT_WISE.START_COLUMN_NUMBER
const END_COLUMN_NUMBER = ATTENDANCE_SUMMARY_CONSTANTS_PROJECT_WISE.END_COLUMN_NUMBER
...
export const attendanceSummaryProjectWiseGenarations = (
...
async function filledDataFormatting(worksheet: any) {
  worksheet.eachRow({ includeEmpty: true }, function (row: any, rowNumber: number) {
    if (rowNumber > COMMON_USES_SUMMERY_PROJECT_WISE.HEADER_ROW) {
...
async function stylingTables(worksheet: any) {
  ...
  const headerRow = worksheet.getRow(COMMON_USES_SUMMERY_PROJECT_WISE.HEADER_ROW)
  ...
  weeksColumns.forEach(function (wk: number) {
    ...
    worksheet.getRow(COMMON_USES_SUMMERY_PROJECT_WISE.FIRST_ROW).fill = {
      type: 'pattern',
      pattern: 'solid',
      fgColor: { argb: COMMON_COLORS.WHITE },
    }
    worksheet.getRow(COMMON_USES_SUMMERY_PROJECT_WISE.FIRST_ROW).border = {
      bottom: { style: 'thin' },
    }
  })
...
async function createheds(searchData: any, worksheet: any) {
  ...
  worksheet.getRow(COMMON_USES_SUMMERY_PROJECT_WISE.HEADER_ROW).values = keyHeads

  for (let ob = 1; ob <= heads.length; ob++) {
    worksheet.getCell(
      `${worksheet.getColumn(ob).letter}${COMMON_USES_SUMMERY_PROJECT_WISE.HEADER_ROW}`
    ).numFmt = heads[ob - 1].style?.numFmt
    worksheet.getCell(
      `${worksheet.getColumn(ob).letter}${COMMON_USES_SUMMERY_PROJECT_WISE.HEADER_ROW}`
    ).alignment = {
...
async function fillData(worksheet: any, data: AttendanceSummaryDataObject[]) {
  let rowNo = COMMON_USES_SUMMERY_PROJECT_WISE.DATA_ROWS_START
  ...
  dataArray?.forEach(function (object: AttendanceSummaryDataObject) {
    ...
    dataRow.push(object.projectName)
    dataRow.push(object.name)
    dataRow.push(object.projectRole)
...
async function addLegendColorsToEntry(
  ...
  switch (dataFlag) {
    ...
    case EXCEL_ATTENDANCE_SUMMARY_DATE_FLAG.HALF_NONWORKING_DAY:
      switch (leaveStatus) {
        // Any Leave (Approved / Applied / Half)
...
async function createExcel(
  ...
  setfilters(START_ROW_FOR_COLOUR_CODE + 5, PROJECT, searchData.projectName, worksheet)

  // Process each row for beautification
  worksheet.eachRow({ includeEmpty: true }, function (row: any) {
    row.eachCell(function (cell: any, colNumber: number) {
      // Set border of each cell
      if (colNumber !== COMMON_USES_SUMMERY_PROJECT_WISE.FIRST_COLUMN) {
        cell.border = {
          top: { style: 'thin' },
          left: { style: 'thin' },
          bottom: { style: 'thin' },
